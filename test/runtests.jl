...
@testset "qfit" begin
    ...
    E0, cntr, Qf = @inferred(RegisterFit.qfit(MismatchArray(num, denom), 1.0e-3))
    @test abs(E0) < eps()
    @test cntr ≈ [1, -2]
    @test Qf ≈ Q

    num = num .+ 5
    E0, cntr, Qf = RegisterFit.qfit(MismatchArray(num, denom), 1.0e-3)
    @test E0 ≈ 5
    @test cntr ≈ [1, -2]
    @test Qf ≈ Q
...
@testset "PAT" begin
    ...
    for i in 1:2
        ...
        @test abs(S[1, 2]) ≈ 1
        @test abs(S[2, 1]) ≈ 1
        @test abs(S[1, 1]) < 1.0e-8
        @test abs(S[2, 2]) < 1.0e-8
    end
